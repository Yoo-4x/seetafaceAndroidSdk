#include <seeta/JNIAgePredictor.h>
...
using NativeClass = seeta::AgePredictor;
...
JNIEXPORT void JNICALL Java_com_seeta_sdk_AgePredictor_construct
  ...
  (JNIEnv *env, jobject self, jobject face, jintArray age)
  {
	jclass self_class = env->GetObjectClass(self);
	jfieldID self_filed_impl = env->GetFieldID(self_class, "impl", "J");
	...
	auto result = native_impl->PredictAge(native_face, native_age);

	if (result)
	{
		env->SetIntArrayRegion(age, 0, 1, &native_age);
	}
  ...
  (JNIEnv *env, jobject self, jobject image, jobjectArray landmarks, jintArray age)
  {
	jclass self_class = env->GetObjectClass(self);
	jfieldID self_filed_impl = env->GetFieldID(self_class, "impl", "J");
	...
	int native_age;
	auto result = native_impl->PredictAgeWithCrop(native_image, native_point5, native_age);
